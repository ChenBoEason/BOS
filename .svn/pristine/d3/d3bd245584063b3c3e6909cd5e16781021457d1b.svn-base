	...
	<div region="center" style="overflow:auto;padding:5px;" border="false">
		<form id="noticebillForm" action="${pageContext.request.contextPath }/noticebillAction_add.action" method="post">
			<table class="table-edit" width="95%" align="center">
				...
				<tr>
					<td>来电号码:</td>
					<td><input id="telbtn" type="text" class="easyui-validatebox" name="telephone"
						required="true" />
						<script type="text/javascript">
							//也可以写成 input[name=telephone]
							$("#telbtn").blur(function(){
								var telphoneNume = this.value;
								//通过ajax发送数据
								var url = "${pageContext.request.contextPath}/noticebillAction_findCustomerByTelephone.action";
								$.post(url, {"telephone":telphoneNume}, function(data){
									if(data != null){
